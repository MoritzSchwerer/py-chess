...
class GameStatus {
   public:
    ...
    uint8_t getStatusPattern() const {
        return isWhite | wKingC << 1 | wQueenC << 2 | bKingC << 3 |
               bQueenC << 4 | enpassant << 5;
    }

    void nextPlayer() { isWhite = !isWhite; }
    ...
    void wMoveRookLeft() { wQueenC = false; }

    void wMoveRookRight() { wKingC = false; }

    void bMoveRookLeft() { bQueenC = false; }

    void bMoveRookRight() { bKingC = false; }

    void enableEnpassant() { enpassant = true; }

    void disableEnpassant() { enpassant = false; }
};
