...
template <bool isWhite> constexpr Bitboard getPawns(const GameState &state) {
  if constexpr (isWhite)
    return state.w_pawn;
  else
    return state.b_pawn;
...
template <bool isWhite> constexpr Bitboard getKnights(const GameState &state) {
  if constexpr (isWhite)
...
template <bool isWhite> constexpr Bitboard getBishops(const GameState &state) {
  if constexpr (isWhite)
...
template <bool isWhite> constexpr Bitboard getRooks(const GameState &state) {
  if constexpr (isWhite)
    return state.w_rook;
  else
    return state.b_rook;
...
template <bool isWhite> constexpr Bitboard getQueens(const GameState &state) {
  if constexpr (isWhite)
...
template <bool isWhite> constexpr Bitboard getKing(const GameState &state) {
  if constexpr (isWhite)
    return state.w_king;
  else
    return state.b_king;
...
template <bool isWhite> constexpr Bitboard secondLastRank() {
  if constexpr (isWhite)
    return RANK_7;
  else
    return RANK_2;
...
template <bool isWhite> constexpr Bitboard secondRank() {
  if constexpr (isWhite)
    return RANK_2;
  else
    return RANK_7;
...
template <bool isWhite> constexpr Bitboard lastRank() {
  if constexpr (isWhite)
    return RANK_8;
  else
    return RANK_1;
...
template <bool isWhite> constexpr Bitboard pawnPush1(Bitboard pawns) {
  if constexpr (isWhite)
    return pawns << 8;
  else
    return pawns >> 8;
...
template <bool isWhite> constexpr Bitboard pawnPush2(Bitboard pawns) {
  if constexpr (isWhite)
    return pawns << 16;
  else
    return pawns >> 16;
...
template <bool isWhite> constexpr Bitboard pawnAttackLeft(Bitboard pawns) {
  if constexpr (isWhite)
    return pawns << 7;
  else
    return pawns >> 9;
...
template <bool isWhite> constexpr Bitboard pawnAttackRight(Bitboard pawns) {
  if constexpr (isWhite)
    return pawns << 9;
  else
    return pawns >> 7;
...
template <bool isWhite> constexpr Bitboard initialRookLeft() {
  if constexpr (isWhite)
    return 1ull;
  else
    return 1ull << 56;
...
template <bool isWhite> constexpr Bitboard initialRookRight() {
  if constexpr (isWhite)
    return 7ull;
  else
    return 7ull << 56;
