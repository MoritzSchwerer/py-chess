...
template <bool isWhite> Bitboard getCheckMask(GameState state) {
  ...
  Bitloop(pawns) {
    ...
    const Bitboard attacks = pawnAttackLeft<!isWhite>(sourceBoard & ~FILE_A) |
...
template <bool isWhite> Bitboard getPinMaskHV(GameState state) {
  const Bitboard enemies = getEnemyPieces<isWhite>(state);
  const Bitboard friendlies = getFriendlyPieces<isWhite>(state);
  ...
  Bitloop(rooks) {
    ...
    const Bitboard kingAttacks =
        Lookup::perSquareXrayRookAttacks[kingSquare][kingBlockIdx] | kingBoard;
...
template <bool isWhite> Bitboard getPinMaskDG(GameState state) {
  const Bitboard enemies = getEnemyPieces<isWhite>(state);
  const Bitboard friendlies = getFriendlyPieces<isWhite>(state);
  const Bitboard kingBoard = getKing<isWhite>(state);
  const uint64_t kingSquare = SquareOf(kingBoard);
...
void getLegalPawnMoves(const GameState &state, Bitboard checkMask,
  ...
  Bitloop(pawnsNoPromo) {
    ...
    Bitloop(targetSquares) {
      ...
      uint64_t flags = 0ull;
      flags |= (targetBoard & pawnAttackLeft<isWhite>(sourceBoard & ~FILE_A)) >>
               targetSquare << 2;
      flags |=
          (targetBoard & pawnAttackRight<isWhite>(sourceBoard & ~FILE_H)) >>
          targetSquare << 2;
...
template <bool isWhite> Bitboard getKingAttacks(const GameState &state) {
  const Bitboard king = getKing<isWhite>(state);
...
template <bool isWhite> Bitboard getSeenSquares(const GameState &state) {
  const Bitboard enemies = getEnemyPieces<isWhite>(state);
  const Bitboard friendlies = getFriendlyPieces<isWhite>(state);
...
Bitboard getEnemyBishopSeenSquaresAfterEnpassant(const GameState &state,
  ...
  const Bitboard blockingPieces = (enemies | friendlies | enpassantSquare) &
                                  ~pawnSquare & ~(1ull << attackingPawnSquare);
...
void getLegalEnpassantCaptures(const GameState &state, Moves &moves) {
  ...
  Bitloop(attackingPawns) {
    ...
    const Bitboard bishopSeenSquares =
        getEnemyBishopSeenSquaresAfterEnpassant<isWhite>(state, sourceSquare);
