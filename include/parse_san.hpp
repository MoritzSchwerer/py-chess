...
uint64_t sanToSquare(std::string squareSAN) {
    if (squareSAN.length() != 2)
        throw std::runtime_error("Error: SAN must be of length 2");
...
Moves getLegalMovesPerPiece(const GameState &state,
    ...
    switch (pieceType[0]) {
        case 'R':
            Movegen::getLegalRookMoves<isWhite>(state, checkMask, pinMaskHV,
                                                pinMaskDG, moves);
            break;
        case 'N':
            Movegen::getLegalKnightMoves<isWhite>(state, checkMask, pinMaskHV,
                                                  pinMaskDG, moves);
            break;
        case 'B':
            Movegen::getLegalBishopMoves<isWhite>(state, checkMask, pinMaskHV,
                                                  pinMaskDG, moves);
            break;
        case 'Q':
            Movegen::getLegalQueenMoves<isWhite>(state, checkMask, pinMaskHV,
                                                 pinMaskDG, moves);
            break;
        case 'K':
            ...
            break;
        default:
            Movegen::getLegalPawnMoves<isWhite>(state, checkMask, pinMaskHV,
                                                pinMaskDG, moves);
            if (state.status.enpassant)
                Movegen::getLegalEnpassantCaptures<isWhite>(state, moves);
            break;
    }
    return moves;
}
...
Move sanToMove(std::string san, const GameState &state) {
    ...
    const Bitboard file =
        match.str(2).empty() ? 0xffffffffffffffff : charToFile(match.str(2)[0]);
    const Bitboard rank =
        match.str(3).empty() ? 0xffffffffffffffff : charToFile(match.str(2)[0]);
    ...
    for (const Move &m : moves) {
        ...
        if (moveTarget == targetSquare && (1ull << moveSource & file & rank)) {
            ...
            if (pieceType.empty() &&
                (1ull << targetSquare) & lastRank<isWhite>()) {
    ...
    if (validMove == 0ull)
        throw std::runtime_error("Error: Didn't find a valid move for: " + san);
    return validMove;
}

template <bool isWhite>
Moves sansToMoves(const std::vector<std::string> &sans,
