  ...
  for (uint64_t ss = 0; ss < 64; ss++) {
    ...
    for (uint64_t i = 0; i < bishopAttackMaskSize; i++) {
      ...
      for (uint64_t ts = ss + 9; ts < 64; ts += 9) {
        ...
        attacks |= targetBoard;
        if ((targetBoard & blockers) | (targetBoard & border))
          break;
      }
      // south-east
  ...
  for (uint64_t ss = 0; ss < 64; ss++) {
    ...
    for (uint64_t i = 0; i < bishopAttackMaskSize; i++) {
      ...
      for (uint64_t ts = ss + 9; ts < 64; ts += 9) {
        ...
        const Bitboard targetBoard = 1ull << ts;
        attacks |= targetBoard;
        if (targetBoard & blockers)
          hitCount++;
        if (hitCount >= 2 || (targetBoard & border))
          break;
      }
      // south-east
      hitCount = 0;
  ...
  for (uint64_t ss = 0; ss < 64; ss++) {
    ...
    for (uint64_t i = 0; i < rookAttackMaskSize; i++) {
      ...
      for (int ts = ss + 8; ts < 64; ts += 8) {
        const Bitboard targetBoard = 1ull << ts;
        attacks |= targetBoard;
        if ((targetBoard & blockers) | (targetBoard & hBorder))
          break;
      }
      // east
      for (int ts = ss + 1; ts < 64 && ts % 8 > 0; ts++) {
        const Bitboard targetBoard = 1ull << ts;
        attacks |= targetBoard;
        if ((targetBoard & blockers) | (targetBoard & vBorder))
          break;
      }
      // south
      for (int ts = ss - 8; ts >= 0; ts -= 8) {
  ...
  for (uint64_t ss = 0; ss < 64; ss++) {
    ...
    for (uint64_t i = 0; i < rookAttackMaskSize; i++) {
      ...
      for (int ts = ss + 8; ts < 64; ts += 8) {
        ...
        if (targetBoard & blockers)
          hitCount++;
        if (hitCount >= 2 || (targetBoard & hBorder))
          break;
      }
      // east
      hitCount = 0;
      for (int ts = ss + 1; ts < 64 && ts % 8 > 0; ts++) {
        ...
        if (targetBoard & blockers)
          hitCount++;
        if (hitCount >= 2 || (targetBoard & vBorder))
          break;
      }
      // south
      hitCount = 0;
...
template <bool isWhite> int8_t getOffsetFromPlane(uint8_t plane) {
  if (isWhite)
